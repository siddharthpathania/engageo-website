/**
 * Country dialing codes for the phone-number picker.
 * `dial` includes the leading '+'. Flag emoji is derived from the ISO-3166
 * alpha-2 code at render time (see `flagEmoji`), so this stays name + iso2 + dial.
 * India is first (default); the rest are alphabetical by name.
 */
export type Country = {
  name: string;
  iso2: string;
...
/** Regional-indicator flag emoji from an ISO-3166 alpha-2 code (e.g. 'IN' → 🇮🇳). */
...
const DIAL_BY_ISO = new Map(COUNTRIES.map((c) => [c.iso2, c.dial]));

/** Dial code (with '+') for an ISO-3166 alpha-2 code; falls back to India. */
export function dialFor(iso2: string): string {
  return DIAL_BY_ISO.get(iso2) ?? '+91';
}
